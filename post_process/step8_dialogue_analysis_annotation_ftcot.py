...
def process_item(item, index, cache):
    ...
    if item_hash in cache:
        ...
        # 验证缓存中的论证分析结果
        response_valid = validate_argument_analysis_result(
            cached_item.get("response_argument_analysis")
        )
        output_valid = validate_argument_analysis_result(
            cached_item.get("output_argument_analysis")
        )
        dialogue_valid = cached_item.get("dialogue_argument_analysis") is not None

        if response_valid and output_valid and dialogue_valid:
            print(f"Item {index} already in cache with valid results, skipping")
            return cached_item
        else:
            print(f"Item {index} cached results are invalid, reprocessing")
...
"""
    ...
    try:
        ...
        dialogue_argument_analysis_result = argument_analysist(input_text)
    except Exception as e:
        dialogue_argument_analysis_result = None
    ...
    try:
        response_argument_analysis_result = response_argument_analysis(
            input_text, last_user + ": " + last_utterance
        )

        # 验证结果
        cnt = 0
        while (
            not validate_argument_analysis_result(response_argument_analysis_result)
            and cnt < 10
        ):
            print(f"Item {index} response analysis result is invalid, retrying...")
            response_argument_analysis_result = response_argument_analysis(
                input_text, last_user + ": " + last_utterance
            )
            cnt += 1
    except Exception as e:
        response_argument_analysis_result = None
    ...
    if (
        dialogue_argument_analysis_result
        and response_argument_analysis_result
        and output_argument_analysis_result
    ):
        item["dialogue_argument_analysis"] = dialogue_argument_analysis_result
        item["response_argument_analysis"] = response_argument_analysis_result
...
def main(
    ...
    # 加载缓存
    cache_file_path = os.path.join(output_dir, f"ftcot_{data_split}.jsonl")
...
if __name__ == "__main__":
    ...
    #     orig_input_fp = f"/home/sunyang/hlt/new_cmv_dataset/final_dataset/list_format/{data_split}_list.jsonl"
    #     output_dir = f"/home/sunyang/hlt/new_cmv_dataset/final_dataset/post_process/ftcot/{data_split}"
    #     test_mode = False
